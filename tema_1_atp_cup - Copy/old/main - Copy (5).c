...
void fEnqueue(Queue* queue, Player** vAdrPlayers)
{
	QueueNode* queueNode = malloc(sizeof(QueueNode));
	queueNode->vAdrPlayers = vAdrPlayers;
	queueNode->next = NULL;
...
void fQueue(Country* vCountries, int* vLocal, Queue* queue)
{
	...
	Player** vAdrPlayers = malloc(sizeof(Player*) * 2);
	for (int iPlayer = 0; iPlayer < country1.nrPlayers; ++iPlayer)
	{
		vAdrPlayers[0] = malloc(sizeof(Player));
		vAdrPlayers[0] = country1.vAdrPlayers[iPlayer];

		for (int jPlayer = 0; jPlayer < country2.nrPlayers; ++jPlayer)
		{
			vAdrPlayers[1] = malloc(sizeof(Player));
			vAdrPlayers[1] = country2.vAdrPlayers[jPlayer];
			fEnqueue(queue, vAdrPlayers);
