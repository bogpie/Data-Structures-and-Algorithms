...
void fQueue(Country** vAdrCountries, int* vLocal, Queue* queue)
{
	Country country1 = *vAdrCountries[0];
	Country country2 = *vAdrCountries[1];
...
void fTournament(StackNode* stackTop)
{
	...
	while (stackTop!=NULL)
	{
		...
		vLocal[0] = vLocal[1] = 0;
		Country** vAdrCountries = malloc(sizeof(Country*) * 2);
		QueueNode* queueNode = malloc(sizeof(QueueNode));

		for (int i = 0; i < 2; ++i)
		{
			Country* adrCountry = stackTop->adrCountry;
			vAdrCountries[i] = adrCountry;
			fPop(&stackTop);
		}
		...
		fQueue(vAdrCountries,vLocal, queue);
