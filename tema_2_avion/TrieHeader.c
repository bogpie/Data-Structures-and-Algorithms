...
void fCountTrie(TrieNode* trieRoot, char* word, int pos) // numararea cuvintelor din arbore; parametrii: radacina, un cuvant care se modifica in apel recursiv si pozitia ultimei litere
{
	if (trieRoot == NULL)
	{
	...
	for (i = 0; i < ALPHABET; ++i)  // pentru fiecare litera din alfabet
	{
		if (trieRoot->vLetters[i] != NULL) // daca sirul se continua in litera cu ordinul i 
		{
			...
			fCountTrie(trieRoot->vLetters[i], word, pos + 1); // apelam recursiv functia, avand acum ca parametrii noul nod, noul cuvant si pozitia anterioara + 1 (am adaugat o litera)
		}
